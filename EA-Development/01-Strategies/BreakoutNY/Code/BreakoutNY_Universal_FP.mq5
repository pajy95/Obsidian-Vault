//+------------------------------------------------------------------+
//|  BreakoutNY_Universal_FP.mq5                                     |
//|  BreakoutNY — EA Universal — FundingPips                         |
//|  Timeframe : M5   Version : 1.0   Abril 2026                    |
//|                                                                  |
//|  Compatible: NAS100 · DJI30 · SP500 · XAUUSD                   |
//|  Cada activo configura sus propios inputs (SL, sesión, días)    |
//|                                                                  |
//|  FEATURES:                                                       |
//|    · Dirección operativa: Solo Compra / Solo Venta / Compra y Venta |
//|    · Modelo de entrada: Confirmación al cierre / Ruptura directa |
//|    · Visual histórico completo con modo de entrada reflejado     |
//|    · ATR filter, filtros de día, parciales opcionales            |
//|    · Módulo visual avanzado (NAS100 v9.1 como fuente)           |
//+------------------------------------------------------------------+
#property copyright "BreakoutNY Universal v1.0 — Jose Yanez"
#property version   "1.00"
#property description "BreakoutNY Universal — NAS100 / DJI30 / SP500 / XAUUSD — FundingPips"
#property strict
...
input int    EntryWindowEnd_Min = 15;   // Minutos de ventana de entrada desde las 14:50 UTC
input int    SessionCloseHour   = 17;   // Hora UTC de cierre forzoso de sesión
input int    SessionCloseMin    = 0;    // Minutos de cierre forzoso

//=== RIESGO =========================================================
input group "=== RIESGO ==="
input double RiskAmountUSD      = 50.0; // Riesgo por trade en USD ($5k=50 · $25k=250 · $100k=1000)

//=== MODELO DE ENTRADA ==============================================
input group "=== MODELO DE ENTRADA ==="
input ENUM_ENTRY_MODEL EntryModel      = ENTRY_CONFIRM_CLOSE; // Modelo de entrada
// CONFIRM_CLOSE: espera cierre de la vela M5 que rompe el rango → filtra mechas falsas
// BREAKOUT_TICK: entra en el primer tick que supera el rango → más rápido, más whipsaws
input int    EntryMaxCandle     = 0;    // Máx vela M5 permitida para el breakout (0 = sin límite)
// Ejemplo: EntryMaxCandle=2 → ignora breakouts en vela 3+ (15:00 UTC en adelante)
// Aplica solo con ENTRY_CONFIRM_CLOSE
...
input int    BE_BufferPct       = 50;   // % del SL para activar BreakEven (ej. 50 = cuando el precio recorre 0.5×SL)
input double TP1_RR             = 1.0;  // TP1: múltiplo del SL (referencia / parcial)
input double TP2_RR             = 2.0;  // TP2: SL se mueve a TP1 al alcanzar este nivel
input double TP3_RR             = 3.0;  // TP3: objetivo final — cierre total de la posición

//=== CIERRES PARCIALES ==============================================
input group "=== CIERRES PARCIALES ==="
input bool   EnablePartials     = false; // Activar cierres parciales en TP1 y TP2
input int    TP1_ClosePct       = 40;   // % del lote a cerrar al alcanzar TP1
input int    TP2_ClosePct       = 40;   // % del lote a cerrar al alcanzar TP2

//=== FILTROS DE RANGO ===============================================
input group "=== FILTROS DE RANGO ==="
input double MinSL_Points       = 25.0;  // Rango mínimo en precio (NAS100=25.0 · DJI30=82.0 · SP500=4.0 · XAUUSD=5.5)
input double MaxSL_Points       = 50.0;  // Rango máximo en precio (NAS100=50.0 · DJI30=200.0 · SP500=15.5 · XAUUSD=15.0)

//=== FILTROS DE DÍA =================================================
input group "=== FILTROS DE DIA ==="
input bool   FilterMonday       = true;  // Operar los lunes
input bool   FilterTuesday      = true;  // Operar los martes
input bool   FilterWednesday    = false; // Operar los miércoles (false = filtrado por defecto)
...
input double ATR_MaxMultiplier  = 2.0;  // Bloquea si ATR_actual > baseline × este valor
input double ATR_MinMultiplier  = 0.3;  // Bloquea si ATR_actual < baseline × este valor
input int    ATR_BaselineDays   = 20;   // Días de lookback para calcular baseline ATR

//=== LOGGING ========================================================
input group "=== LOGGING ==="
input bool   EnableCSV          = true;
input string CSVFileName        = "BreakoutNY_Universal_trades";
...
input int    MagicNumber        = 202400; // Magic number único por instancia del EA

//===================================================================
//  ESTADO GLOBAL
//===================================================================

// Prefijo visual: generado dinámicamente en OnInit para no colisionar entre instancias
string   VIS_PFX = "BNYV_";

// Specs del símbolo
double   g_cachedVPP  = 0;   // Value Per Point ($/ punto / lot)
...
// Breakout pendiente (modo CONFIRM_CLOSE)
bool     g_brkPending     = false;
int      g_brkDir         = 0;         // 1=buy, -1=sell
datetime g_brkBarTime     = 0;
int      g_brkCandleNum   = 0;         // Vela M5 en que se detectó el breakout
...
// NY open time (para EntryMaxCandle)
...
double GetValuePerPoint()
{
   ...
   // Método 1: TickValue/TickSize válidos
   if(tv > 0 && ts > 0)
      vpp = tv / ts;
   // Método 2: TickValue=0 (bug brokers CFD — FundingPips NAS100/DJI30/SP500)
   // Para CFD de índice: VPP = ContractSize (equivalencia tv/ts = CS×pt/pt = CS)
   else if(cs > 0)
   {
      vpp = cs;
      Print("INFO VPP: TickValue=0. Fallback: VPP=ContractSize=", cs);
   }
   else
   {
      Print("ERROR VPP: TV=", tv, " TS=", ts, " CS=", cs, " PT=", pt);
      return 0;
   }

   PrintFormat("VPP=$%.4f | TickValue=%.6f | TickSize=%.6f | ContractSize=%.2f",
               vpp, tv, ts, cs);
   return vpp;
}
...
{
   if(g_cachedVPP <= 0 || slPts <= 0) return 0;
   ...
   // Verificación de margen — calcula margen POR 1 LOT y deriva el máximo permitido
   // (mismo método que NAS100 v9.1 — evita la fórmula circular)
   double freeMargin  = AccountInfoDouble(ACCOUNT_MARGIN_FREE);
   double marginPerLot = 0;
   if(!OrderCalcMargin(ORDER_TYPE_BUY, _Symbol, 1.0, ask, marginPerLot))
   {
      // Fallback si OrderCalcMargin falla: estimar margen como 5% del nocional
      marginPerLot = ask * SymbolInfoDouble(_Symbol, SYMBOL_TRADE_CONTRACT_SIZE) * 0.05;
   }

   if(marginPerLot > 0)
   {
      double maxLotsByMargin = MathFloor((freeMargin * 0.90) / marginPerLot / vStep) * vStep;
      maxLotsByMargin = MathMax(vMin, MathMin(vMax, maxLotsByMargin));
      if(lots > maxLotsByMargin)
      {
         PrintFormat("AVISO MARGEN: lots=%.3f excede margen libre=$%.2f (max=%.3f lots) — reduciendo",
                     lots, freeMargin, maxLotsByMargin);
         lots = maxLotsByMargin;
      }
   }

   double realRisk = lots * slPts * g_cachedVPP;
   PrintFormat("LotCalc | SL=%.4f | VPP=$%.4f/pt/lot | Raw=%.4f | Lots=%.3f | Risk=$%.2f | Margin/lot=$%.2f | FreeMargin=$%.2f",
               slPts, g_cachedVPP, lRaw, lots, realRisk, marginPerLot, freeMargin);
   return lots;
}
...
int OnInit()
{
   ...
   if(MinSL_Points >= MaxSL_Points)
      { Alert("MinSL_Points debe ser menor que MaxSL_Points"); return INIT_FAILED; }

   // Prefijo visual único por símbolo — evita colisión entre instancias
   string symClean = _Symbol;
   StringReplace(symClean, ".", "");
   StringReplace(symClean, " ", "");
   VIS_PFX = "BNYV_" + StringSubstr(symClean, 0, 6) + "_";
   ...
   if(g_cachedVPP <= 0) { Alert("FATAL: no se pudo determinar ValuePerPoint"); return INIT_FAILED; }
      ...
      if(g_csvHandle != INVALID_HANDLE)
      {
         ...
         if(FileTell(g_csvHandle) == 0)
            FileWrite(g_csvHandle,
                      "Fecha","Dia","Dir","Lotes","VPP","Riesgo$",
                      "Entry","SL","BE","TP1","TP2","TP3",
                      "SL_pts","EntryModel","BrkCandle","ATR");
   ...
   Print("╔══════════════════════════════════════════════════════╗");
   PrintFormat("║  BreakoutNY Universal v1.0 — %s", _Symbol);
   Print("╚══════════════════════════════════════════════════════╝");
   PrintFormat("  VPP=$%.6f | Riesgo=$%.2f | CierreUTC=%02d:%02d",
   ...
   PrintFormat("  _Point=%.5f | MinSL=%.4f | MaxSL=%.4f (unidades de precio)",
               _Point, MinSL_Points, MaxSL_Points);
   Print("  Tabla de lotes:");
   double vMin  = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
   double vStep = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
   double vMax  = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX);
   double slTest[] = {MinSL_Points, (MinSL_Points + MaxSL_Points) / 2.0, MaxSL_Points};
   for(int i = 0; i < 3; i++)
   {
      double lr = RiskAmountUSD / (slTest[i] * g_cachedVPP);
      double lf = MathMax(vMin, MathMin(vMax, MathFloor(lr / vStep) * vStep));
      PrintFormat("    SL=%.4f -> lots=%.3f -> riesgo=$%.2f",
...
void OnTick()
{
   ...
   //--- Formar el rango (solo una vez por día, a partir de las 14:50 UTC)
   ...
   //--- Ventana de entrada: 14:50 UTC hasta 14:50 + EntryWindowEnd_Min
   ...
   {
      // FASE B — Confirmación: nueva vela abierta distinta a la que rompió
      // Evaluar primero para no perder el primer tick de la vela de confirmación
      ...
      // FASE A — Detección: buscar primer tick que supera el rango
      if(!g_brkPending)
      {
         ...
         {
            // Filtro de vela máxima (EntryMaxCandle)
            int candleNum = 1;
...
//===================================================================
//  TryFormRange — busca velas 14:35 / 14:40 / 14:45 UTC con iBarShift
//===================================================================
...
void TryFormRange(MqlDateTime &utcDT)
{
   ...
   // Validar que las velas corresponden al horario esperado (tolerancia 5 min)
   ...
   // g_slDistance en unidades de precio (puntos reales del índice / precio del activo)
   // Consistente con el cálculo de lotes: RiskUSD / (slDistance × VPP)
   // MinSL_Points y MaxSL_Points deben expresarse en la misma unidad
   //   NAS100 FundingPips : 1 punto real = precio 1.0  → MinSL=25.0  MaxSL=50.0
   //   DJI30  FundingPips : 1 punto real = precio 1.0  → MinSL=82.0  MaxSL=200.0
   //   SP500  FundingPips : 1 punto real = precio 1.0  → MinSL=4.0   MaxSL=15.5
   //   XAUUSD FundingPips : 1 punto real = precio 0.01 → MinSL=5.5   MaxSL=15.0
   g_slDistance = g_rangeHigh - g_rangeLow;

   // Filtro de rango
   if(g_slDistance < MinSL_Points)
   {
      PrintFormat("RANGE SKIP: dist=%.4f < MinSL=%.4f", g_slDistance, MinSL_Points);
      if(VIS_Enable) VisUpdatePanel("", 0, 0, false, true,
         StringFormat("Rango filtrado (%.2f < %.2f)", g_slDistance, MinSL_Points));
      return;
   }
   if(g_slDistance > MaxSL_Points)
   {
      PrintFormat("RANGE SKIP: dist=%.4f > MaxSL=%.4f", g_slDistance, MaxSL_Points);
      if(VIS_Enable) VisUpdatePanel("", 0, 0, false, true,
         StringFormat("Rango filtrado (%.2f > %.2f)", g_slDistance, MaxSL_Points));
      return;
   }
   ...
   PrintFormat("RANGE VALIDO: H=%.4f L=%.4f dist=%.4f lots=%.3f riesgo=$%.2f",
               g_rangeHigh, g_rangeLow, g_slDistance, g_cachedLots,
               g_cachedLots * g_slDistance * g_cachedVPP);
}
...
void ExecuteTrade(int dir)
{
   ...
   if(dir == 1)  // BUY
   {
      ...
      if(trade.Buy(g_cachedLots, _Symbol, ask, g_originalSL, g_tp3Level, "BreakoutNY_BUY"))
      {
         ...
         g_beActivated = false; g_tp2Hit = false; g_tp1Hit = false;
         g_tradeDir = "buy";
         PrintFormat("BUY %.3f | SL=%.5f BE=%.5f TP1=%.5f TP2=%.5f TP3=%.5f",
   ...
   else  // SELL
   {
      ...
      if(trade.Sell(g_cachedLots, _Symbol, bid, g_originalSL, g_tp3Level, "BreakoutNY_SELL"))
      {
         ...
         g_beActivated = false; g_tp2Hit = false; g_tp1Hit = false;
         g_tradeDir = "sell";
         PrintFormat("SELL %.3f | SL=%.5f BE=%.5f TP1=%.5f TP2=%.5f TP3=%.5f",
                     g_cachedLots, g_originalSL, g_beLevel, g_tp1Level, g_tp2Level, g_tp3Level);
         vis_hasTrade = true; VisRedrawToday();
         LogTrade("SELL");
      }
...
void ManagePosition()
{
   ...
   if(posTyp == POSITION_TYPE_BUY)
   {
      ...
      // TP2 — SL sube a TP1 + parcial opcional
   ...
   else if(posTyp == POSITION_TYPE_SELL)
   {
      ...
      // TP2 — SL baja a TP1 + parcial opcional
...
void LogTrade(string dir)
{
   ...
   FileWrite(g_csvHandle,
             ...
             DoubleToString(g_slDistance, 2), modStr,
...
//===================================================================
//  ██████╗  MÓDULO VISUAL
//  Fuente: NAS100 v9.1 — adaptado para prefijo dinámico y
//          soporte de ambos modelos de entrada en VisDrawDay
//===================================================================
...
//===================================================================
//  VisDrawDay — dibuja rango + niveles de un día
//  Modelo de entrada se refleja en el punto de entrada visual:
//    CONFIRM_CLOSE → entry = open de la vela siguiente al breakout
//    BREAKOUT_TICK → entry = precio de ruptura (close de la vela)
//===================================================================
...
void VisDrawDay(const MqlRates &R[], int total, datetime dayUTC)
{
   ...
   for(int i = total - 1; i >= 0; i--)
   {
      ...
      if(bullOK || bearOK)
      {
         ...
         // El punto de entrada visual coincide con el modelo de entrada configurado
         if(EntryModel == ENTRY_CONFIRM_CLOSE)
   ...
   if(broke)
   {
      ...
      // Flecha de dirección — 2 velas antes del rango
      ...
      // Etiqueta de modelo de entrada encima del rango
...
//===================================================================
//  VisRedrawAll — redibujar histórico completo
//===================================================================
...
//===================================================================
//  VisRedrawToday — redibujar solo el día actual
//===================================================================
...
//===================================================================
//  VisUpdatePanel — panel superior derecho
//===================================================================
                    ...
                    bool hasTrade, bool isFiltered, string filterReason)
{
   ...
   else
   {
      ...
      line1 = "BreakoutNY v1.0  |  " + _Symbol + "  |  " + dayStr;
...
//===================================================================
//  VisDrawMottoPanel — panel inferior izquierdo
//===================================================================
...
void VisDrawMottoPanel()
{
   ...
   ObjectSetString(0,  nm3, OBJPROP_TEXT,
      StringFormat("BreakoutNY Universal v1.0  ·  %s FundingPips  ·  Magic %d  ·  Jose Yanez",
