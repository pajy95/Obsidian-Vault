//+------------------------------------------------------------------+
//|   BreakoutNY_Calibrador_GBPUSD.mq5                               |
//|   Calibrador de Rangos NY — GBPUSD — FundingPips                 |
//|                                                                   |
//|   PROPÓSITO:                                                      |
//|   Analiza los últimos N años de datos M5 del GBPUSD y            |
//|   calcula la distribución estadística del rango de las 3 velas   |
//|   14:35 / 14:40 / 14:45 UTC (rango de apertura NY).             |
//|                                                                   |
//|   GBPUSD SPECS:                                                   |
//|   Dígitos: 5 | pipSize = _Point * 10                             |
//|   El rango se mide en PIPS (1 pip = 0.0001)                      |
//|                                                                   |
//|   OUTPUT:                                                         |
//|   1. Log en el journal de MT5 con todos los percentiles          |
//|   2. CSV exportado: BreakoutNY_Calibracion_GBPUSD.csv            |
//|                                                                   |
//|   USO:                                                            |
//|   1. Compilar y ejecutar como Script en gráfico GBPUSD M5        |
//|   2. Revisar Journal: buscar "RESULTADO FINAL"                   |
//|   3. Usar P25 como MinSL_Pips y P75 como MaxSL_Pips en el EA   |
//+------------------------------------------------------------------+
#property copyright "BreakoutNY Calibrador GBPUSD v1.0"
...
input double MaxSL_Test        = 30.0;   // Pips máximos de referencia
...
void OnStart()
{
   ...
   Print("╔══════════════════════════════════════════════════════╗");
   Print("║  BreakoutNY Calibrador NY Range — GBPUSD v1.0       ║");
   Print("╚══════════════════════════════════════════════════════╝");
   ...
   for(int i = 2; i < copied - 1; i++)
   {
      ...
      // GBPUSD: filtro razonable — rango > 200 pips en 15 min es anomalía
   ...
   Print("  Cargar estos valores en BreakoutNY_Universal_FP en gráfico GBPUSD");
   Print("════════════════════════════════════════════════════════");

   // ── Exportar CSV ──────────────────────────────────────────────
   if(!ExportCSV) return;

   string csvName = "BreakoutNY_Calibracion_GBPUSD.csv";
   ...
   FileWrite(fh, "ValuePerPip",  DoubleToString(valuePerPip, 2)+" $/pip/lote", "", "", "", "");
