...
//|  BreakoutNY Strategy — SPX500 (US SPX 500 Cash Index)            |
//|  FundingPips Challenge | MetaTrader 5 | M5                        |
//|  MagicNumber: 202403                                             |
//|                                                                  |
//|  VALIDATED: IS PF=1.636 MaxDD=7.14% | OOS PF=1.567 MaxDD=5.56% |
//|  OOS retention: 95.8% of IS PF  (threshold ≥70%) — APROBADO    |
//|                                                                  |
//|  VPP FIX: ContractSize*Point = 50*0.01 = $0.50/point/lot       |
//|  FOR $5k CHALLENGE: RiskAmountUSD = 50 (1% of $5,000)          |
//+------------------------------------------------------------------+
#property copyright "BreakoutNY v1.0 SP500 FundingPips — Jose Yanez"
#property version   "1.00"
#property description "IS PF=1.636 | OOS PF=1.567 | OOS retention 95.8%"
#property strict
...
input int     SessionCloseHour   = 21;   // Cierre forzoso hora UTC — validado IS/OOS
...
input int     BE_BufferPct       = 130;  // % del SL antes de activar BE — validado IS/OOS
input double  TP1_RR             = 1.0;  // TP1 ratio (informacional)
...
input bool    FilterWednesday    = false; // Mantener false — validado en backtests SP500
input bool    FilterThursday     = true;
input bool    FilterFriday       = true;

//=== RANGE FILTER =======================================================
input group   "=== RANGE FILTER ==="
input double  MinSL_Points       = 400.0;   // P25 calibrador SP500 2021-2024 (P25=735, ajustado IS)
input double  MaxSL_Points       = 1550.0;  // P90 calibrador SP500 2021-2024 (P90=2280, ajustado IS)
input bool    ConfirmOnClose     = true;    // SIEMPRE true — confirmación al cierre de vela

//=== ATR FILTER =========================================================
input group   "=== ATR FILTER ==="
input bool    ATR_FilterEnable   = true;
input int     ATR_Period         = 14;
input double  ATR_MaxMultiplier  = 2.0;
input double  ATR_MinMultiplier  = 0.3;
input int     ATR_BaselineDays   = 20;

//=== MISC ===============================================================
input group   "=== MISC ==="
input bool    EnableCSV          = true;
input string  CSVFileName        = "BreakoutNY_v1_SP500_trades";
input int     MagicNumber        = 202403;  // SP500=202403 · DJI30=202402 · NAS100=202401

//=== VISUAL =============================================================
input group "=== VISUAL ==="
input bool   VIS_Enable         = true;
input int    VIS_LookbackDays   = 25;
input int    VIS_ExtendBars     = 50;
input bool   VIS_ShowBE         = true;
input bool   VIS_ShowLabel      = true;
input color  VIS_ColRange       = C'14,25,52';
input color  VIS_ColRangeBorder = C'55,115,215';
input color  VIS_ColSL          = C'220,50,70';
input color  VIS_ColEntry       = C'195,208,230';
input color  VIS_ColBE          = C'0,195,218';
input color  VIS_ColTP1         = C'228,188,38';
input color  VIS_ColTP2         = C'228,128,28';
input color  VIS_ColTP3         = C'28,208,128';
...
#define VIS_PFX "BNYV_SP_"   // Prefijo único SP500 — no colisiona con DJI30 "BNYV_"
...
double GetValuePerPoint()
{
   ...
   double vpp = cs * pt;   // SP500: 50 * 0.01 = $0.50
   if(vpp <= 0) {
...
int OnInit()
{
   ...
   if(SessionCloseHour>23)              { Alert("SessionCloseHour debe ser 0-23"); return INIT_FAILED; }
   ...
   Print("=== BreakoutNY v1.0 SP500 FundingPips ===");
   PrintFormat("  VPP=$%.6f | Riesgo=$%.2f | CierreSession=%02d:00 UTC",
               g_cachedVPP, RiskAmountUSD, SessionCloseHour);
   PrintFormat("  BE=%.0f%% TP=%.1fR/%.1fR/%.1fR | Dias: Lun=%s Mar=%s Mie=%s Jue=%s Vie=%s",
   ...
   double slArr[] = {400, 600, 800, 1100, 1550};
   ...
   PrintFormat("  IS: PF=1.636 MaxDD=7.14%% | OOS: PF=1.567 MaxDD=5.56%% | Retencion=95.8%%");
   Print("==========================================");
...
void TryFormRange(MqlDateTime &utcDT)
{
   ...
   if(g_slDistance < MinSL_Points) {
      PrintFormat("RANGE SKIP: dist=%.0fpts < MinSL=%.0fpts", g_slDistance, MinSL_Points);
   ...
   if(g_slDistance > MaxSL_Points) {
      PrintFormat("RANGE SKIP: dist=%.0fpts > MaxSL=%.0fpts", g_slDistance, MaxSL_Points);
      if(VIS_Enable) VisUpdatePanel("",0,0,false,true,
   ...
   g_rangeFormed = true;
   vis_dayTag = VisMakeDayTag();
   PrintFormat("RANGE VALIDO: H=%.2f L=%.2f dist=%.0fpts lotes=%.3f riesgo=$%.2f",
...
bool CheckATRFilter()
{
   ...
   if(cur[0] < baseline*ATR_MinMultiplier) {
      PrintFormat("RANGE SKIP: ATR bajo (%.4f < %.4f baseline x %.1f)", cur[0], baseline, ATR_MinMultiplier);
      if(VIS_Enable) VisUpdatePanel("",0,0,false,true,"ATR filtrado (bajo)");
      return false;
   }
   if(cur[0] > baseline*ATR_MaxMultiplier) {
      PrintFormat("RANGE SKIP: ATR alto (%.4f > %.4f baseline x %.1f)", cur[0], baseline, ATR_MaxMultiplier);
...
//+------------------------------------------------------------------+
//  MODULO VISUAL — portado íntegramente desde DJI30 v4.1
//  Adaptaciones SP500: prefijo VIS_PFX, etiquetas, métricas OOS
//+------------------------------------------------------------------+
...
void VisDrawDay(const MqlRates &R[], int total, datetime dayUTC)
{
   ...
   VisSegment(VIS_PFX+"RH_"+tag, t35, tRangeEnd, rH, rH,
              VIS_ColRangeBorder, STYLE_SOLID, 2, true);
   VisSegment(VIS_PFX+"RL_"+tag, t35, tRangeEnd, rL, rL,
              VIS_ColRangeBorder, STYLE_SOLID, 2, true);

   VisText(VIS_PFX+"LRHI_"+tag, t35, rH, "  RANGE HIGH",
           VIS_ColRangeBorder, 8, ANCHOR_LEFT_LOWER, false);
   VisText(VIS_PFX+"LRLO_"+tag, t35, rL, "  RANGE LOW",
   ...
   if(broke)
   {
      ...
      // Línea vertical en vela de ruptura
      ...
      // Marcador de cierre de sesión (SCH=21 UTC)
...
void VisRedrawAll()
{
   ...
   PrintFormat("VIS: Redibujando %d dias historicos (ultimos %d dias)",
...
//===================================================================
//  PANEL INFERIOR — identidad del EA + frases
//===================================================================
void VisDrawMottoPanel()
{
   ...
   ObjectSetString(0,  nm3, OBJPROP_TEXT,
      StringFormat("BreakoutNY v1.0  ·  SPX500 FundingPips  ·  Magic %d  ·  Jose Yanez  ·  IS PF=1.636  ·  OOS PF=1.567  ·  Retencion 95.8%%",
                   (int)MagicNumber));
